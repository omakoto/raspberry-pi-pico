...
// Custom vprintf handler to duplicate log lines across UART and USB CDC ACM.
static int dual_vprintf(const char* fmt, va_list args) {
    ...
    // 1. Output to default UART0 stdout
    int ret = vprintf(fmt, args);
    std::fflush(stdout);

    // 2. Output to USB CDC ACM when the host has enumerated and mounted the device
