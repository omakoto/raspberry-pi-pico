#!/bin/bash
# Flashes the compiled w5500-lan-test firmware, partition table, and FATFS image to the Seeed Studio XIAO ESP32-S3.
...
if [[ ! -f "build/w5500-lan-test.bin" || ! -f "build/storage.bin" ]]; then
...
if [[ -n "$PORT" ]]; then
    echo "Flashing w5500-lan-test to port ${PORT}..."
    idf.py -p "$PORT" flash "$@"
else
