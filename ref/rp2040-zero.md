...
```text
        ...
        Button                                |      |  RP2040  |      |                             Button
                                              ...
                                              |         (GP16)         |
                                              |                        |
                            5V  (VBUS Input) -| [L1]              [R1] |- GP0 / UART0 TX / I2C0 SDA / SPI0 RX
                               GND  (Ground) -| [L2]              [R2] |- GP1 / UART0 RX / I2C0 SCL / SPI0 CS
                   3V3  (3.3V Regulated Out) -| [L3]     RP2040   [R3] |- GP2 / I2C1 SDA / SPI0 SCK
 GP29 / ADC3 / UART0 RX / I2C0 SCL / SPI0 TX -| [L4]      ZERO    [R4] |- GP3 / I2C1 SCL / SPI0 TX
 GP28 / ADC2 / UART0 TX / I2C0 SDA / SPI1 RX -| [L5]   (23-Pin    [R5] |- GP4 / UART1 TX / I2C0 SDA / SPI0 RX
            GP27 / ADC1 / I2C1 SCL / SPI1 TX -| [L6]   U-Layout)  [R6] |- GP5 / UART1 RX / I2C0 SCL / SPI0 CS
           GP26 / ADC0 / I2C1 SDA / SPI1 SCK -| [L7]              [R7] |- GP6 / I2C1 SDA / SPI0 SCK
                   GP15 / I2C1 SCL / SPI1 TX -| [L8]              [R8] |- GP7 / I2C1 SCL / SPI0 TX
                  GP14 / I2C1 SDA / SPI1 SCK -| [L9]              [R9] |- GP8 / UART1 TX / I2C0 SDA / SPI1 RX
                                              +------------------------+
                                                |    |    |    |    |
                                               13   12   11   10    9
                                                |    |    |    |    |
                                                |    |    |    |    +-- GP9 / UART1 RX / I2C0 SCL / SPI1 CS
                                                |    |    |    +------- GP10 / I2C1 SDA / SPI1 SCK
                                                |    |    +------------ GP11 / I2C1 SCL / SPI1 TX
                                                |    +----------------- GP12 / UART0 TX / I2C0 SDA / SPI1 RX
                                                +---------------------- GP13 / UART0 RX / I2C0 SCL / SPI1 CS
```
...
| **2** | `GP2` | GPIO | *(UART0 CTS)*| **I2C1 SDA** | **SPI0 SCK** | PWM1 A | — |
| **3** | `GP3` | GPIO | *(UART0 RTS)*| **I2C1 SCL** | **SPI0 TX** | PWM1 B | — |
| **4** | `GP4` | GPIO | **UART1 TX** | **I2C0 SDA** | **SPI0 RX** | PWM2 A | — |
| **5** | `GP5` | GPIO | **UART1 RX** | **I2C0 SCL** | **SPI0 CSn** | PWM2 B | — |
| **6** | `GP6` | GPIO | *(UART1 CTS)*| **I2C1 SDA** | **SPI0 SCK** | PWM3 A | — |
| **7** | `GP7` | GPIO | *(UART1 RTS)*| **I2C1 SCL** | **SPI0 TX** | PWM3 B | — |
| **8** | `GP8` | GPIO | **UART1 TX** | **I2C0 SDA** | **SPI1 RX** | PWM4 A | — |
...
| Silk Label | GPIO | Default | UART | I2C | SPI | PWM | Analog |
| :---: | :---: | :--- | :--- | :--- | :--- | :--- | :---: |
| **9** | `GP9` | GPIO | **UART1 RX** | **I2C0 SCL** | **SPI1 CSn** | PWM4 B | — |
| **10**| `GP10`| GPIO | *(UART1 CTS)*| **I2C1 SDA** | **SPI1 SCK** | PWM5 A | — |
| **11**| `GP11`| GPIO | *(UART1 RTS)*| **I2C1 SCL** | **SPI1 TX** | PWM5 B | — |
...
| **28**| `GP28`| ADC2 | **UART0 TX** | **I2C0 SDA** | **SPI1 RX** | PWM6 A | **ADC2** |
| **29**| `GP29`| ADC3 | **UART0 RX** | **I2C0 SCL** | **SPI0 TX** | PWM6 B | **ADC3** |
