#!/bin/bash
#
# Builds the otg-lan-test firmware for Raspberry Pi Pico boards.
#
# Usage:
#   ./00-build.sh [-b <pico|pico_w|pico2|pico2_w>] [-c] [cmake-configure-options]
#
# Default board is pico (RP2040, non-W). -c wipes build/ first. Extra arguments are passed to cmake.
#
...
echo "Configuring otg-lan-test for board '${BOARD}'..."
cmake -B build \
    -DPICO_BOARD="${BOARD}" \
    -DPICO_SDK_PATH="${PICO_SDK_PATH}" \
    "${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}"

echo "Building otg-lan-test for board '${BOARD}'..."
cmake --build build -j"$(nproc)"

echo "Build successful! Target outputs generated in build/:"
ls -lh build/otg-lan-test.elf build/otg-lan-test.uf2
