#!/bin/bash
# Flashes the otg-lan-test firmware to a Raspberry Pi Pico / Pico 2 / Pico W / Pico 2 W.
#
# Supports both:
# 1. Direct copy to a mounted BOOTSEL mass-storage drive (RPI-RP2 or RP2350).
# 2. Automated flashing via picotool.
#
# A running board is rebooted into BOOTSEL mode automatically, either through picotool
# (reset interface) or by setting 1200 baud on the CDC serial console.
...
FIRMWARE_UF2="build/otg-lan-test.uf2"
...
# 1. Attempt to reboot the device into BOOTSEL mode via picotool (reset interface)
...
# 2. Attempt to reboot the device into BOOTSEL mode via the CDC serial console (1200 baud touch).
#    The console is interface 2 of the composite device, hence the "-if02" by-id suffix.
for cdc_dev in /dev/serial/by-id/usb-omakoto_otg-lan-test*-if02 /dev/ttyACM*; do
    if [[ -e "$cdc_dev" ]]; then
        echo "Found Pico serial console at $cdc_dev. Requesting reboot to BOOTSEL mode..."
        python3 -c "
import os, termios
try:
...
echo "" >&2
echo "To put the board into BOOTSEL mode manually:" >&2
...
echo "  4. Re-run ./01-install.sh, or drag and drop build/otg-lan-test.uf2 onto the RP2350 / RPI-RP2 volume." >&2
exit 1
