/*
 * Controller State Engine Implementation for nsbackend-pico.
 * Handles parsing and merging of all Switch gamepad inputs.
 */
...
static inline int64_t current_time_us() {
...
void ControllerState::execute_command(const std::string& cmd_line) {
    ...
    if (auto_release) {
        ...
        int64_t target_time = current_time_us() + delay_us;
