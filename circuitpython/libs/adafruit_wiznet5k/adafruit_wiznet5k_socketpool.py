...
class Socket:
    ...
    def accept(
        ...
        while True:
            status = self._status
            if status == wiznet5k.adafruit_wiznet5k.SNSR_SOCK_ESTABLISHED:
                break
            if status not in {
                wiznet5k.adafruit_wiznet5k.SNSR_SOCK_LISTEN,
                ...
                # If the socket transitioned away from listen or handshake state,
                # close the hardware socket to return to SOCK_CLOSED, then re-enter listen.
                self._interface.socket_close(self._socknum)
                self.listen()
        ...
        dest_ip = self._interface.remote_ip(self._socknum)
        dest_port = self._interface.remote_port(self._socknum)
        addr = (dest_ip, dest_port)

        current_socknum = self._socknum
        # Create a new socket object and swap socket nums for the connected client
        ...
        # Defer re-entering listen state until the subsequent accept() invocation so only
        # one hardware socket is active on this port at any given time, preventing hardware
        # packet routing conflicts.
