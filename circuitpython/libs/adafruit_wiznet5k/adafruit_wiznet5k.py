...
class WIZNET5K:
    ...
    def socket_open(self, socket_num: int, conn_mode: int = _SNMR_TCP) -> None:
        ...
        if self._read_snsr(socket_num) != SNSR_SOCK_CLOSED:
            # The W5500 OPEN command is only valid when the socket status is SOCK_CLOSED.
            # Close the socket if it is currently in any other state to guarantee SOCK_CLOSED.
